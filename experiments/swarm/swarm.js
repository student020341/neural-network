...
class Thing {
    constructor(bounds, env) {
        ...
        this.brain.name = "Swarm Agent";
        this.brain.inputLabels = ["Food Left", "Food Right", "Food Above", "Food Below"];
        this.brain.outputLabels = ["Move Horizontal", "Move Vertical"];
    }
...
canvas.addEventListener("click", (e) => {
    ...
    for (let i = 0; i < swarm.things.length; i++) {
        const thing = swarm.things[i];
        if (Math.hypot(clickX - thing.x, clickY - thing.y) < thing.size * 2) {
            visualizer.inspect(`Agent #${i + 1}`, thing, (t) => t.brain);
            break;
        }
    }
});
...
const logic = (dt) => {
    ...
    // Auto-track first agent if none tracked or current target removed
    if (visualizer.providers.length === 0 && swarm.things.length > 0) {
        visualizer.track("Agent #1", swarm.things[0], (t) => t.brain);
    }
}
